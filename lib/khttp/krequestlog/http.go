...
func NewHandler(next http.Handler, mods ...Modifier) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if opts.LogStart {
			printLogLine(opts, "HTTP START origin=%s method=%s path=%s", origin, method, path)
		}
		...
		if opts.LogEnd {
			...
			if opts.LogFormat == "apache" {
				...
				printLogLine(opts, "%s - - [%s] \"%s %s %s\" %d %d \"%s\" \"%s\" %v",
					origin,
				...
				printLogLine(opts, "HTTP END origin=%s method=%s path=%s status=%d size=%d duration=%v", origin, method, path, status, sw.length, duration)
			}
		}
	})
}
