...
func NewHandler(next http.Handler, mods ...Modifier) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if opts.LogStart {
			opts.Printer("HTTP START method=%s path=%s", method, path)
		}
		...
		if opts.LogEnd {
			...
			if opts.LogFormat == "apache" {
				...
				opts.Printer("HTTP END method=%s path=%s status=%d size=%d duration=%v", method, path, status, sw.length, duration)
			}
		}
	})
}
