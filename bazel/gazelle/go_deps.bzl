...
load("@gazelle//internal:go_repository.bzl", "go_repository")
load(
...
def _get_patch_args(path, module_overrides):
    ...
    return ["-p{}".format(override)] if override != None else ["-p0"]
...
def _go_deps_impl(module_ctx):
    ...
    for module in module_ctx.modules:
        ...
        for from_file_tag in from_file_tags:
            ...
            # This is a repo-owned extension, not generic upstream Gazelle wiring.
            # Keep the extension user's go.mod replacement semantics intact even
            # when this module is consumed as a non-root dependency.
            #
            # "First in wins" preserves the root module's ability to override
            # transitive replacement directives.
            for mod_path, mod in go_mod_replace_map.items():
                if not mod_path in replace_map:
                    replace_map[mod_path] = mod

            if not module.is_root and not getattr(module_ctx, "is_isolated", False):
...
""".format(
    ...
    for path, module in module_resolutions.items():
        if hasattr(module, "module_name") or path in _SHARED_REPOS:
...
def _get_sum_from_module(path, module, sums):
    ...
    if entry not in sums:
        if module.raw_version == COMPARES_HIGHEST_SENTINEL:
            ...
            print("No sum for {}@{} found, run bazel run".format(path, module.raw_version), Label("@rules_go//go"), "-- mod tidy to generate it")
            return None
